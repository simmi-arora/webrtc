...
function RTCMultiConnection(roomid, forceOptions) {
    ...
    connection.removeStream = function(streamid) {
        ...
        if (!stream) {
            webrtcdev.warn('No such stream exists.', streamid);
            return;
        }
                ...
                connection.getScreenConstraints(function(error, screen_constraints) {
                    if (error) {
                        if (error === 'PermissionDeniedError') {
                            ...
                            if (connection.enableLogs) {
                                webrtcdev.error('User rejected to share his screen.');
                            }
                            return;
                        }
                        return alert(error);
    ...
    connection.setCustomSocketHandler = function(customSocketHandler) {
        alert(" customSocketHandler" + customSocketHandler);
        if (typeof SocketConnection !== 'undefined') {
    ...
    connection.onUserIdAlreadyTaken = function(useridAlreadyTaken, yourNewUserId) {
        if (connection.enableLogs) {
            webrtcdev.warn('Userid already taken.', useridAlreadyTaken, 'Your new userid:', yourNewUserId);
        }
